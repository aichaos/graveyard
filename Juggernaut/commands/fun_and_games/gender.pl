#      .   .             <CKS Juggernaut>
#     .:...::     Command Name // !gender
#    .::   ::.     Description // Look up the gender of a first name.
# ..:;;. ' .;;:..        Usage // !gender <name>
#    .  '''  .     Permissions // Public
...
sub gender {
	...
	if (length $msg > 0) {
		...
		my $src = LWP::Simple::get "http://cgi.sfu.ca/~gpeters/cgi-bin/pear/gender.php?firstname=$msg" or return "Error: Could not access data.";
		$src =~ s/\n//ig;
		$src =~ s/<(.|\n)+?>//g;
		$src = lc($src);
