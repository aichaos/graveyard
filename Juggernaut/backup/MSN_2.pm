...
sub display_picture {
	...
	if(!$file) {
		...
		if((-e $file) && $file =~ /png$/) {
			...
			carp "Could not find the file $file, or it is not a PiNG file";
			return 0;	
		}
	}
}
...
sub add_emoticon {
	...
	if((-e $file) && $file =~ /png$/) {
		...
		my $object = '<msnobj Creator="'.$self->GetMaster->{Handle}.'" Size="'.(-s $file).'" Type="3" Location="'.$shortcut.'.tmp" Friendly="AAA=" SHA1D="'.$sha1d.'"';
...
sub _add_to_list
{
  ...
  $client->send( "ADC", "$list N=$email F=$email" ) if $self->isVer('MSNP10') and ($list ne 'AL');
...
=head1 CKS JUGGERNAUT

This specific copy of MSN.pm was taken from the local lib of CKS Juggernaut,
created by Chaos AI Technology - http://chaos.kirsle.net/
