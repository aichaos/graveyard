...
sub AUTOLOAD
{
	...
	$self->error( "method $MSN::Notification::AUTOLOAD not defined" ) if( $self->{Msn}->{AutoloadError} );
}
...
sub connect
{
	...
	$self->{Socket} = new IO::Socket::INET(
														  ...
														  PeerPort => $port,
														  Proto	  => 'tcp'
														  ) or return $self->serverError( "Connection error: $!" );
...
sub setStatus
{
  ...
  if (defined $self->{DPLocation} && exists $self->{Objects}->{$self->{DPLocation}} ) {
	...
	$self->send( 'CHG', $status . " " . $cid . " " . $object);
}
...
sub addEmoticon
{
	...
	{
		return $self->error( "Could not find the file '$filename', or it is not a PiNG file" );
	}	
}
...
{
	my $self = shift;
	my $handle = shift || return $self->error( "Need to send the handle of the person you call" );
	my $message = shift || '';
...
sub ping
{
	...
	if( time >= $self->{PingTime} + $self->{PingIncrement} )
	{
		...
		# move this code outside of this if block?? if no pong has come, it's pointless to send another ping, as above
...
#================================================
# This value is only stored on the server and has no affect
...
#	$self->debug( "'$email', '$friendly', '$bitmask', '$guid', '$group'" );
	...
	if ($current == $total)
	{
		...
		foreach my $key (keys %$RL)
		{
			...
			{
				# This contact needs to be allowed
				$self->allowContact( $key );
			}
		}
...
sub CMD_NLN
{
	...
	$self->setContactStatus( $data[1], $data[0] );
}
...
sub CMD_FLN
{
	...
	$self->setContactStatus( $data[0], 'FLN' );
}
...
sub CMD_ILN
{
	...
	$self->setContactStatus( $data[2], $data[1] );
 }
...
sub CMD_ADC
{
	...
	if( $list eq 'RL' )		# a user is adding us to their contact list (our RL list)
	{
		...
		my $do_add = $self->{Msn}->call_event( $self, "ContactAddingUs", $handle ) || 1;				  
		$self->allowContact( $handle ) if( $do_add );
	}
...
sub CMD_XFR
{
	...
	elsif( $data[1] eq 'SB' )
	{
		if( defined $self->{Calls}->{$data[0]}->{Handle} )
		{
			...
			$switchboard->connectXFR( $data[4], $self->{Calls}->{$data[0]}->{Handle}, $self->{Calls}->{$data[0]}->{Message} );
		}
		else
		{
...
sub CMD_RNG
{
	...
	my $do_accept = $self->{Msn}->call_event( $self, "Ring", $user, uri_unescape($friendly) ) || 1;
