...
MSN Protocol Connector Version 1.3.6
...
Version 1.3.4 to 1.3.5 (Jan 25, 2004)
     ...
     ejh - added code to skip some MSGs with Content-Type application
...
use constant VERSION => "MSN Module v. 1.3.6"; 
...
sub complain 
 { 
      ...
      push(@callerstack, ($line, $file)); 

      while(($pack, $file, $line, $subname) = caller($i++)) 
      { push(@callerstack, ($subname, $line, $file)); } 
      push(@callerstack, "main"); 

      ($line, $file, $subname) = (shift(@callerstack), shift(@callerstack), shift(@callerstack));  
      print "    in $subname line $line <$file>\n";  
			
      while(@callerstack)  
     {  
          ($line, $file, $subname) = (shift(@callerstack), shift(@callerstack), shift(@callerstack));  
          print "     called from $subname line $line <$file>\n";  
     }   
}
...
sub addcontact {
    ...
    unless ($email) {
           carp "email address required to add";
           return 0;
    }

    $client->{Lists}->{FL}->{$email} = 1;
    
    $client->send( "ADC", "FL N=$email" ) if $self->isVer('MSNP10');
    $client->send( "ADD", "FL $email $email" ) if $self->isVer('MSNP9');
...
sub remcontact {
    ...
    unless ($email) {
           carp "email address required to rem";
           return 0;
    }

    delete $client->{Lists}->{FL}->{$email};
    
    $client->send( "REM", "FL $email" );
...
sub do_one_loop
{
      ...
      foreach $fh (@ready)
      {
        ...
        elsif ( $cmd eq 'BYE' )
        {
            ...
            {
                print "Closing " . $self->{Socket}->fileno() . "\n";
        ...
        {
            #$self->writelog( "Group Listing - $data[0]\n\n" );
        }
        elsif ($cmd eq 'SYN')
        {         
        ...
        {     
				my ($email,$friendly,$c,$bitmask,$group) = @data;	# default to MSNP10 - if $self->isVer('MSNP10');
            ...
            if ($current == $total)
            {
                #$self->writelog( "Listing Complete\n\n" );
                my $RL = $self->{Lists}->{RL};
                my $AL = $self->{Lists}->{AL};
                my $BL = $self->{Lists}->{BL};
