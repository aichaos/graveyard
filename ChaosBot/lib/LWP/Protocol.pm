...
{
    my($scheme, $ua) = @_;
    if ($use_mshttps && ($scheme eq 'https'))
    {
	    $scheme = 'mshttps';
    }
    my $impclass = LWP::Protocol::implementor($scheme);

    if (!$impclass && ($scheme eq 'https') && ($^O eq 'MSWin32'))
    {
	    my $new_scheme = 'mshttps';
	    $impclass = LWP::Protocol::implementor($new_scheme);
	    if ($impclass)
	    {
		    $scheme = $new_scheme;
		    $use_mshttps = 1;
